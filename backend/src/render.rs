...
impl Renderer {
    ...
    pub fn new(canvas: HtmlCanvasElement) -> Result<Renderer, JsValue> {
        ...
        let sprite_fsh = compile_shader(&ctx, WebGl2RenderingContext::FRAGMENT_SHADER, sprite_fsh)?;
    ...
    fn render_inner(&self, beatmap: &Beatmap) -> Result<()> {
        ...
        let position_attribute_location = self.ctx.get_attrib_location(&program, "position");
