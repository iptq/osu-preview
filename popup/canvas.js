...
const processHitObjects = (hitObjects, timingPoints, SV) => {
  for (let i = 0; i < hitObjects.length; i += 1) {
    ...
    if (isSlider(object)) {
      const { ms_per_beat: beatDuration } = timingPoints.find(
        (e) => e.time <= object.time
      );
...
const playPreview = (
  ...
  // backendRenderer.render(beatmap);
  ...
  const animate = (currentTime) => {
    const time = seeking
      ? mapStartTime
      : currentTime - startTime + mapStartTime;
    // eslint-disable-next-line no-param-reassign
    playbackTimeElement.innerText = `${toTimeString(
      Math.min(time, lastTime)
    )} / ${toTimeString(lastTime)}`;
