...
const getMainBPM = (timingPoints, hitObjects) => {
  ...
  timingPoints.filter((point) => point.ms_per_beat > 0)
    .forEach((point, i, arr) => {
      ...
      const duration = endTime - point.time;
      if (!bpms.has(point.ms_per_beat)) {
        bpms.set(point.ms_per_beat, 0);
      }
      bpms.set(point.ms_per_beat, bpms.get(point.ms_per_beat) + duration);
    });
  const mainBpm = 60000 / [...bpms.entries()]
    .reduce(([mainMsPerBeat, maxCount], [msPerBeat, count]) => (count > maxCount ? [msPerBeat, count] : [mainMsPerBeat, maxCount]), [0, 0])[0];
  return mainBpm;
};
...
export default class ManiaRenderer {
  ...
  render(time) {
    const {
      ctx, keyCount, mainBPM, timingPoints,
    } = this;
